...
object BoundaryTraverslaOfTree extends App{
  ...
  def printBoundaryLeft[A](tree: BinaryTree[A]):Unit =  {
    //println("Inside printBoundaryLeft"); 
    tree match {
      case EmptyTree =>
  ...
  def printBoundaryRight[A](tree: BinaryTree[A]):Unit = {
    //println("Inside printBoundaryRight");
    tree match {
      case EmptyTree =>
      case Node(v, l, r) => (l, r) match {
        case (EmptyTree, EmptyTree) =>
  ...
  def boundaryTraversalRecu[A](tree: BinaryTree[A]):Unit = tree match {
    case EmptyTree => 
    case Node(v,l,r) => 
      print(v + " , ")
      printBoundaryLeft(l)
